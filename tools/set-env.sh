...
case $VENDOR in
  ...
  hygon)
    ...
    export DTK_ENV=""
    for f in /opt/dtk-2504/env.sh /opt/dtk/env.sh /usr/local/dtk/env.sh /opt/dtk-*/env.sh /usr/local/dtk-*/env.sh; do
      if [ -f "$f" ]; then
        export DTK_ENV="$f"
        source "$f" || true
    ...
    if [ -z "$DTK_ENV" ]; then
      echo "WARNING: no DTK env.sh found under /opt/dtk-2504, /opt/dtk*, /usr/local/dtk*. torch will not see the DCUs."
    fi
