...
# Keep shapes small so this doubles as a smoke test for the Hygon DCU backend.
_HYGON_AXPY_SHAPES = [(1024,), (5333,), (65536,)]


@pytest.fixture(autouse=True)
def _require_hygon_cpu_ref(request):
    if flag_blas.vendor_name != "hygon":
        pytest.skip("test_hygon_axpy only runs on the Hygon DCU backend")
    if request.config.getoption("--ref") != "cpu":
        pytest.skip("test_hygon_axpy only supports --ref cpu")
...
@pytest.mark.hygon
@pytest.mark.axpy
@pytest.mark.parametrize("shape", _HYGON_AXPY_SHAPES)
@pytest.mark.parametrize("alpha", SCALARS)
def test_hygon_axpy_saxpy(shape, alpha):
    n = shape[0]
    incx = 1
    incy = 1
